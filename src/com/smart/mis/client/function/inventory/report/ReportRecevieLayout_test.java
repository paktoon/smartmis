...
public class ReportRecevieLayout_test extends VLayout{
	...
	public ReportRecevieLayout_test(final User currentUser) {
...
//        searchItem.setUseTextField(true);
        ...
        text.setContents("รายงานการรับสินค้าและวัตถุดิบ");
        ...
        reportListGrid.setData(getReceivedRecord());
	}
...
//		printButton.hide();
		...
		select_date = DateTimeFormat.getFormat("yyyy-M-dd").parse(DateTimeMapping.getRealYear(select_year) + "-" + select_month + "-" + select_day);
		
		//reportListGrid.fetchData(new Criterion("created_date", OperatorId.EQUALS, select_date));
		for (ListGridRecord record : reportListGrid.getRecords()) {
			reportListGrid.removeData(record);
		}
		reportListGrid.setData(getReceivedRecord());
...
//		printButton.hide();
	...
	private Record[] getReceivedRecord() {
		TransferDS.getInstance().refreshData();
		PurchaseOrderDS.getInstance().refreshData();
		
		//status = "2_received"
		//received_status = "2_received"
		//received_date = select_date
		AdvancedCriteria transfer_criteria = new AdvancedCriteria(OperatorId.AND, new Criterion[]{
    		      new Criterion("status", OperatorId.EQUALS, "2_received"),
    		      new Criterion("received_date", OperatorId.EQUALS, select_date)
    		  });
		AdvancedCriteria purchase_criteria = new AdvancedCriteria(OperatorId.AND, new Criterion[]{
  		      new Criterion("received_status", OperatorId.EQUALS, "2_received"),
  		      new Criterion("received_date", OperatorId.EQUALS, select_date)
  		  });
		
		Record[] transferred = TransferDS.getInstance().applyFilter(TransferDS.getInstance().getCacheData(), transfer_criteria);
		Record[] purchased = PurchaseOrderDS.getInstance().applyFilter(PurchaseOrderDS.getInstance().getCacheData(), purchase_criteria);

		ArrayList<Record> collection = new ArrayList<Record>();
		collection.addAll(Arrays.asList(transferred));
		collection.addAll(Arrays.asList(purchased));

		return collection.toArray(new Record[] {});
	}
