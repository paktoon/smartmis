...
public class CreateSaleWindow extends EditorWindow{
...
//        }
		...
		final ListGrid quoteListGrid = new ListGrid() {
			...
			protected String getCellCSSText(ListGridRecord record, int rowNum, int colNum) { 
				if (getFieldName(colNum).equals("produce_amount")) {
					if (record.getAttributeAsInt("produce_amount") != 0) {
...
//		System.out.println(cachedData.length);
...
//		quoteListGrid.setUseAllDataSourceFields(false);
		quoteListGrid.setData(addInventoryFields(cachedData));
...
//        quoteItemCell_7.setShowGridSummary(true);
        quoteItemCell_7.setIncludeInRecordSummary(false);
...
//        if (edit) quoteItemCell_6.setCanEdit(true);
        ...
        quoteItemCell_8.setIncludeInRecordSummary(false);
        
        ListGridNumberField quoteItemCell_sum = new ListGridNumberField("sum_price", 100);
        quoteItemCell_sum.setTitle("ราคารวม (บาท)");
        ...
        quoteListGrid.setFields(quoteItemCell_1, quoteItemCell_2, quoteItemCell_6, quoteItemCell_7, quoteItemCell_8, quoteItemCell_3, quoteItemCell_5 , quoteItemCell_sum);
...
//			Integer credit = (Integer) customer.getField("credit").getValue();
			...
			System.out.println("CreateSaleWindow create saleorder for " + cid + " " + cus_name + " " + payment_model + " " + credit + " " + cus_type + " " + bus_type + " " + cus_group + " " + zone);
...
//										ListGridRecord update_quote = QuotationData.createStatusRecord(quote_id, quote_status, "");
	...
	Record[] addInventoryFields(Record[] records) {
		ProductDS.getInstance().refreshData();
		ArrayList<Record> temp = new ArrayList<Record>();
		for (Record item : records) {
			String pid = item.getAttributeAsString("pid");
			Double price = item.getAttributeAsDouble("price");
			Record[] selectedProduct = ProductDS.getInstance().applyFilter(ProductDS.getInstance().getCacheData(), new Criterion("pid", OperatorId.EQUALS, pid));
			Record product = selectedProduct[0];
			Integer quote_amount = item.getAttributeAsInt("quote_amount");
			Integer remain = product.getAttributeAsInt("remain");
			Integer produce_amount = quote_amount > remain ? quote_amount - remain:0;
			item.setAttribute("remain", remain);
			item.setAttribute("produce_amount", produce_amount);
			item.setAttribute("sum_price", price * quote_amount);
			temp.add(item);
		}
