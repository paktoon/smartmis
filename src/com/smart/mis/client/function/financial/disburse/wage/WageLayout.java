...
public class WageLayout extends VLayout {
	
	public WageLayout(final User currentUser){
		...
		searchForm.setDataSource(WageDS.getInstance());
		searchForm.setUseAllDataSourceFields(false);
		searchForm.setGroupTitle("ค้นหาคำขอเบิกค่าจ้างผลิต");
		
		final TextItem planText = new TextItem("wage_id", "รหัสค่าจ้างผลิต");
		...
		final ListGrid orderListGrid = new EditorListGrid(new WageViewWindow(), currentUser);
		...
		orderListGrid.setDataSource(WageDS.getInstance());
		...
		IButton searchButton = new IButton("ค้นหาคำขอเบิกค่าจ้างผลิต");
...
//        });
//		
//		IButton createOrderButton = new IButton("ออกคำสั่งขัดและติดพลอย");
