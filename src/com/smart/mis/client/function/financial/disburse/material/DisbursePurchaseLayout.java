package com.smart.mis.client.function.financial.disburse.material;
...
public class DisbursePurchaseLayout extends VLayout {
	
	public DisbursePurchaseLayout(final User currentUser){
		...
		final SelectItem statusSelected = new SelectItem("payment_status", "สถานะ");
		statusSelected.setWrapTitle(false);
		//statusSelected.setValueMap("รอผลิต", "กำลังผลิต", "พร้อมนำส่ง", "อยู่ระหว่างนำส่ง", "นำส่งแล้ว");
		statusSelected.setValueMap(PurchaseOrderStatus.getValuePaymentMap());
		...
		saleListGrid.setInitialSort(new SortSpecifier[]{ 
                new SortSpecifier("payment_status", SortDirection.ASCENDING),
                new SortSpecifier("created_date", SortDirection.DESCENDING)  
        });
		saleListGrid.setUseAllDataSourceFields(false);
		saleListGrid.setGroupByField("payment_status");
		...
		ListGridField status = new ListGridField("payment_status");
		...
		ListGridField iconField = new ListGridField("paidPurchaseOrderField", "เรียกดู", 50);
		
		saleListGrid.setFields(status,sale_id, quote_id, cus_name, total_amount, netInclusive, created_date, iconField);
		
		saleListGrid.hideField("payment_status");
