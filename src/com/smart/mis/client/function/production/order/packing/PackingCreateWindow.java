...
public class PackingCreateWindow {
...
//			smith_name.setOptionCriteria(new Criterion("type", OperatorId.EQUALS, "ขัดและติดพลอยแมกกาไซต์"));
...
//		dateForm.setFields(fromDate, toDate, deliveryDate);
		...
		final IButton printButton = new IButton("ออกคำสั่งบรรจุสินค้า");
...
//	}
	...
	public void createCreateOrder(DynamicForm planForm, ListGrid orderListGrid, Date sent_date, Date due_date, final User currentUser, final ListGridRecord previousRecord) {
		...
		for (ListGridRecord item : all){
			...
			Record[] selectedProcess = ProcessListDS.getInstance(pid).applyFilter(ProcessListDS.getInstance(pid).getCacheData(), new Criterion("type", OperatorId.EQUALS, "4_packing"));
...
//            }  
            ...
            protected Canvas getExpansionComponent(final ListGridRecord record) {  
    			...
    			for (Record material : selectedMaterialProcess) {
    				materialList.add(PackingMaterialData.createRecord(material, sent_amount));
    			}
