...
public class PurchaseRequestTabSet extends TabSet {

	Boolean allowApproved;
	User currentUser;
	
	public PurchaseRequestTabSet(Boolean allow, User user) {
		...
		RequestCreateTab createTab = new RequestCreateTab();
		RequestReviseTab reviseTab = new RequestReviseTab();
		
		addTab(createTab.getCreateTab(user));
		addTab(reviseTab.getReviseTab(user));
		
		if (this.allowApproved) {
			RequestApproveTab approveTab = new RequestApproveTab();
